<?php
	...
	/*Validar Usuário*/
	if(  $_SESSION['user_nivel'] != 'aluno' AND $_SESSION['user_tipo'] == 0 ){
...
$InputSQL = new db();
$InputSQL->query( "SELECT a.aluno_id, a.aluno_nome, a.aluno_cpf, a.aluno_status, a.aluno_registro, c.class_nome
                   FROM aluno as a
                   LEFT JOIN classe as c
                   ON a.classe_class_id = c.class_id
                   WHERE a.aluno_tipo = 1
                   AND c.class_id = (SELECT classe_class_id FROM aluno WHERE aluno_id = '".$_SESSION['user_id']."')
                   " );
$InputSQL->execute();
...
<a class="btn btn-success" href="<?php echo $url?>!/aluno/cadastro">Novo Professor</a>
      ...
      <tbody>
        ...
        if( !empty($InputSQL->row()) ){
          foreach( $InputSQL->row() AS $row ){
            ?>
            <tr class="odd gradeX <?php if( empty($row['aluno_status']) ) echo 'font-weight-bold'; ?>">
              ...
              <td><?php echo empty($row['aluno_status']) ? 'Em Aberto' : 'Registrado'?></td>
              <td align="right" >
                <!-- <a href="<?php echo $url?>!/aluno/editar-professor/<?php echo $row['aluno_id']?>" class="btn btn-sm btn-outline-success"><i class="fas fa-vote-yea"></i></a> -->
