...
$InputSQL = new db();
$InputSQL->query( "SELECT classe.*, escola.*
                   FROM classe
                   LEFT JOIN escola
                   ON classe.escola_esc_id = escola.esc_id
                   WHERE classe.escola_esc_id = '".$_SESSION['user_id']."'
                   " );
$InputSQL->execute();

?>

<a class="btn btn-success" href="<?php echo $url?>!/classe/cadastrar">Nova Classe</a>

<hr>
<h2 class="mb-3">Classe para Professores<small> (<?php echo $InputSQL->rowCount()?>)</small></h2>
<hr>
        ...
        if( !empty($InputSQL->row()) ){
          foreach( $InputSQL->row() AS $row ){
            ...
            <tr class="odd gradeX ">
              ...
              <td align="right" >
                <a href="<?php echo $url?>!/classe/editar/<?php echo $row['class_id']?>" class="btn btn-sm btn-outline-success"><i class="fas fa-vote-yea"></i></a>
                <!-- <a href="<?php echo $url?>!/curso/deletar/<?php echo $row['class_id']?>&titulo=<?php echo $row['cat_curso_nome']?>" class="btn btn-sm btn-outline-danger"><i class="far fa-trash-alt"></i></a> -->
              </td>
            </tr>
            <?php
          }
